...
# Orden alfabético usado por baseline_mlp.py al cargar features del CSV
# Este es el orden que el modelo espera
FEATURE_ORDER_ALPHABETIC = ["A", "Ab", "B", "Bb", "C", "D", "Db", "E", "Eb", "F", "G", "Gb"]

# Mapeo de librosa chroma_cqt (C, C#, D, D#, E, F, F#, G, G#, A, A#, B) 
# a las notas en NOTE_ORDER (C, Db, D, Eb, E, F, Gb, G, Ab, A, Bb, B)
LIBROSA_TO_NOTE_ORDER = {
    "C": "C", "C#": "Db", "D": "D", "D#": "Eb", 
    "E": "E", "F": "F", "F#": "Gb", "G": "G", 
    "G#": "Ab", "A": "A", "A#": "Bb", "B": "B"
}
...
def reorder_chroma_to_alphabetic(chroma_vec: np.ndarray) -> np.ndarray:
    """
    Reordena el vector de cromas de librosa al orden alfabético usado por baseline_mlp.py.
    
    librosa devuelve: [C, C#, D, D#, E, F, F#, G, G#, A, A#, B]
    baseline_mlp.py espera (orden alfabético): [A, Ab, B, Bb, C, D, Db, E, Eb, F, G, Gb]
    
    Args:
        chroma_vec: vector de 12 cromas en orden de librosa
    
    Returns:
        vector reordenado al orden alfabético
    """
    librosa_order = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"]
    
    # Crear diccionario: nota -> valor
    chroma_dict = {note: chroma_vec[i] for i, note in enumerate(librosa_order)}
    
    # Mapear a notas con bemoles
    chroma_mapped = {}
    for lib_note, value in chroma_dict.items():
        note = LIBROSA_TO_NOTE_ORDER.get(lib_note, lib_note)
        chroma_mapped[note] = value
    
    # Reordenar al orden alfabético
    reordered = np.array([chroma_mapped[note] for note in FEATURE_ORDER_ALPHABETIC], dtype=np.float32)
    return reordered
...
def aggregate_chroma_over_interval(chroma: np.ndarray, times: np.ndarray, t0: float, t1: float) -> np.ndarray:
    ...
    if s > 1e-8:
        vec = vec / s
    # Reordenar al orden alfabético que espera el modelo
    vec = reorder_chroma_to_alphabetic(vec)
...
def infer_on_audio(
    ...
    while i < n_beats:
        ...
        # vec ya está reordenado al orden alfabético (FEATURE_ORDER_ALPHABETIC)
        rows.append({"t_start": t0, "t_end": t1, **{f"chroma_{n}": float(vec[k]) for k, n in enumerate(FEATURE_ORDER_ALPHABETIC)}})
        feats.append(vec)
        i += beats_per_segment
...
def main():
    ...
    out_prefix_pred = out_prefix.parent / (out_prefix.stem + "_predicted")
    save_outputs(df_pred, out_prefix=out_prefix_pred)
    print(f"✅ Guardado: {out_prefix_pred.with_suffix('.lab')}")
